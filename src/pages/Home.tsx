...
import { SEO, PersonStructuredData } from '../components'
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (subIndex === currentWord.length + 1 && index !== words.length - 1 && !reverse) {
      setReverse(true)
      return
    }
    ...
    const timeout = setTimeout(
      ...
      Math.max(reverse ? 75 : subIndex === currentWord.length ? 1000 : 150, Math.floor(Math.random() * 350))
    )
  ...
  return (
    <>
      <SEO
        title="Home"
        description="Justin Nappi - Developer, Designer, and Entrepreneur. Building creative solutions with modern technologies."
        url="/"
      />
      ...
      <main className="bg-primary">
        <section className="flex flex-col text-left justify-center min-h-screen pt-8 px-8">
          ...
          >
            {`${currentWord.substring(0, subIndex)}${blink ? '|' : ' '}`}
          </motion.h1>
